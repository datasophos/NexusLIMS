...
class CDCSUtils:
    """Utility class for interacting with CDCS (Configurable Data Curation System).

    This class provides methods for querying, downloading, and deleting records
    ...
    def get_workspace_id() -> int:
        ...
        # workspace)
        endpoint = urljoin(CDCSUtils.get_cdcs_url(), "rest/workspace/read_access")
    ...
    def get_template_id() -> str:
        ...
        endpoint = urljoin(
            CDCSUtils.get_cdcs_url(), "rest/template-version-manager/global"
        )
        r = nexus_req(endpoint, "GET", token_auth=settings.NX_CDCS_TOKEN)
        if r.status_code in (HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN):
            msg = (
    ...
    def delete_record(record_id: str):
        ...
        """
        endpoint = urljoin(CDCSUtils.get_cdcs_url(), f"rest/data/{record_id}")
        ...
        if response.status_code != HTTPStatus.NO_CONTENT:
            ...
            _logger.error(
                "Received error while deleting %s:\n%s", record_id, response.text
            )
        return response
    ...
    def search_records(
        ...
        if keyword is not None:
            endpoint = urljoin(CDCSUtils.get_cdcs_url(), "rest/data/query/keyword/")
            payload = {
                "query": keyword,
            ...
            endpoint = urljoin(CDCSUtils.get_cdcs_url(), "rest/data/query/")
            # Build query payload
    ...
    def download_record(record_id: str) -> str:
        ...
        """
        endpoint = urljoin(CDCSUtils.get_cdcs_url(), f"rest/data/download/{record_id}/")
    ...
    def upload_record_content(xml_content: str, title: str) -> tuple[Any, int | None]:
        ...
        """
        endpoint = urljoin(CDCSUtils.get_cdcs_url(), "rest/data/")

        payload = {
            "template": CDCSUtils.get_template_id(),
            "title": title,
        ...
        record_url = urljoin(CDCSUtils.get_cdcs_url(), f"data?id={record_id}")
        wrk_endpoint = urljoin(
            CDCSUtils.get_cdcs_url(),
            f"rest/data/{record_id}/assign/{CDCSUtils.get_workspace_id()}",
        )
    ...
    def upload_record_files(
        ...
        *,
        ...
        for f in tqdm(files_to_upload) if progress else files_to_upload:
            ...
            title = f_path.stem
            response, record_id = CDCSUtils.upload_record_content(xml_content, title)
