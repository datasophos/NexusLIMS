...
class TestEDAXSPCExtractor:
    ...
    def test_leo_edax_spc(self):
        ...
        assert meta[0]["nx_meta"]["Azimuthal Angle (deg)"] == pytest.approx(0.0)
        assert meta[0]["nx_meta"]["Live Time (s)"] == pytest.approx(30.000002)
        assert meta[0]["nx_meta"]["Detector Energy Resolution (eV)"] == pytest.approx(
            125.16211,
        )
        assert meta[0]["nx_meta"]["Elevation Angle (deg)"] == pytest.approx(35.0)
        assert meta[0]["nx_meta"]["Channel Size (eV)"] == 5
        assert meta[0]["nx_meta"]["Number of Spectrum Channels"] == 4096
        assert meta[0]["nx_meta"]["Stage Tilt (deg)"] == -1.0
        assert meta[0]["nx_meta"]["Starting Energy (keV)"] == pytest.approx(0.0)
        assert meta[0]["nx_meta"]["Ending Energy (keV)"] == pytest.approx(20.475)
    ...
    def test_leo_edax_msa(self):
        ...
        assert meta is not None
        assert meta[0]["nx_meta"]["Azimuthal Angle (deg)"] == pytest.approx(0.0)
        assert meta[0]["nx_meta"]["Amplifier Time (μs)"] == "7.68"
        assert meta[0]["nx_meta"]["Analyzer Type"] == "DPP4"
        assert meta[0]["nx_meta"]["Beam Energy (keV)"] == pytest.approx(10.0)
        ...
        assert meta[0]["nx_meta"]["Live Time (s)"] == pytest.approx(30.0)
        ...
        assert meta[0]["nx_meta"]["Real Time (s)"] == pytest.approx(0.0)
        assert meta[0]["nx_meta"]["Energy Resolution (eV)"] == "125.2"
        assert meta[0]["nx_meta"]["Signal Type"] == "EDS"
        assert meta[0]["nx_meta"]["Active Layer Thickness (cm)"] == "0.1"
        assert meta[0]["nx_meta"]["Be Window Thickness (cm)"] == pytest.approx(0.0)
        assert meta[0]["nx_meta"]["Dead Layer Thickness (cm)"] == pytest.approx(0.03)
        assert meta[0]["nx_meta"]["EDAX Time"] == "10:14"
        assert meta[0]["nx_meta"]["EDAX Title"] == ""
        assert meta[0]["nx_meta"]["TakeOff Angle (deg)"] == "35.5"
        assert meta[0]["nx_meta"]["Stage Tilt (deg)"] == "-1.0"
